/**
 * Post repair script — fixes categorySlug on existing posts.
 *
 ...
 *
 * Usage:
 *   MONGODB_CONNECTION_URI=<uri> node backend/scripts/fixPosts.js          # dry run
 *   MONGODB_CONNECTION_URI=<uri> node backend/scripts/fixPosts.js --write  # apply
 * Or via npm script:
 *   npm --prefix backend run seed:fix-posts
 *   npm --prefix backend run seed:fix-posts -- --write
 */
...
run().catch((err) => {
    console.error('fixPosts failed:', err);
    process.exit(1);
});
